# [Yazi](https://github.com/sxyazi/yazi)

> Yazi is a blazing-fast terminal file manager written in Rust, designed around asynchronous I/O. It focuses on high performance, extensibility, and rich terminal UI features, making file exploration and management in the command line faster and more convenient for power users. Key highlights include full asynchronous I/O, real-time task management, multiple image and preview protocols, a plugin system, and tight integrations with common developer tooling. [sxyazi/yazi]

> Yazi是一个以Rust编写的快速终端文件管理器，设计围绕异步I/O。它专注于高性能、可扩展性和丰富的终端用户界面特性，使得在命令行中的文件浏览和管理对高级用户来说更快速和便利。主要亮点包括完全的异步I/O、实时任务管理、多个图像和预览协议、插件系统，以及与常用开发工具的紧密集成。 [sxyazi/yazi]

## install

```sh
# Arch
sudo pacman -S yazi
# Windows 10
scoop install yazi
```

## config

[^1]

```sh
# Arch
mkdir ~/.config/yazi/
# Windows 10
mkdir %AppData%\yazi\config
```

## command

```sh
ya pkg add <pkg>
ya pkg upgrade
```

## reference

- #document [Yazi](https://yazi-rs.github.io/docs/installation)
- #keymap [Keybindings](https://yazi-rs.github.io/docs/quick-start#keybindings)

## cross-reference

### later

- [whoosh-yazi.md](/bin/yazi/whoosh-yazi.md)

### cache

- [duckdb-yazi.md](/bin/yazi/duckdb-yazi.md)

## resource

### mark

- #theme [Flexoki Dark Flavor for Yazi](https://github.com/gosxrgxx/flexoki-dark.yazi)
- #theme [Flexoki Light Flavor for Yazi](https://github.com/gosxrgxx/flexoki-light.yazi)
- [custom-shell.yazi](https://github.com/AnirudhG07/custom-shell.yazi)
- [epub-preview.yazi](https://github.com/kirasok/epub-preview.yazi)
- [fr.yazi](https://github.com/lpnh/fr.yazi)
- [Hevi Preview Plugin for Yazi](https://github.com/lihuax-me/hevi.yazi)
- [lazygit.yazi](https://github.com/Lil-Dank/lazygit.yazi)
- [mediainfo.yazi](https://github.com/boydaihungst/mediainfo.yazi)
- [piper.yazi](https://github.com/yazi-rs/plugins/tree/main/piper.yazi)
- [recycle-bin.yazi](https://github.com/uhs-robert/recycle-bin.yazi)
- [sxiv.yazi](https://github.com/NoponyAsked/sxiv.yazi)

### later

- [ffmpeg-stats.yazi](https://github.com/grimandgreedy/ffmpeg-stats.yazi)
- [font-sample.yazi](https://github.com/AminurAlam/yazi-plugins/tree/main/font-sample.yazi)
- [git.yazi](https://github.com/yazi-rs/plugins/tree/main/git.yazi)
- [open-with-cmd.yazi](https://github.com/Ape/open-with-cmd.yazi)
- [preview-audio.yazi](https://github.com/AminurAlam/yazi-plugins/tree/main/preview-audio.yazi)
- [preview-cbz.yazi](https://github.com/AminurAlam/yazi-plugins/tree/main/preview-cbz.yazi)
- [preview-typst.yazi](https://github.com/AminurAlam/yazi-plugins/tree/main/preview-typst.yazi)
- [toggle-pane.yazi](https://github.com/yazi-rs/plugins/tree/main/toggle-pane.yazi)
- [office.yazi](https://github.com/macydnah/office.yazi)
- [chmod.yazi](https://github.com/yazi-rs/plugins/tree/main/chmod.yazi)
- [mount.yazi](https://github.com/yazi-rs/plugins/tree/main/mount.yazi)
- [sshfs.yazi](https://github.com/uhs-robert/sshfs.yazi)

### cache

- [thunar-bulk-rename.yazi](https://github.com/boydaihungst/thunar-bulk-rename.yazi)
- [compress.yazi](https://github.com/KKV9/compress.yazi)
- [krita-preview.yazi](https://github.com/walldmtd/krita-preview.yazi)
- [ouch.yazi](https://github.com/ndtoan96/ouch.yazi)

[^1]: [Yazi - Configuration](https://yazi-rs.github.io/docs/configuration/overview/)

## annex

```sh
ya pkg add gosxrgxx/flexoki-dark
ya pkg add gosxrgxx/flexoki-light
ya pkg add AnirudhG07/custom-shell
ya pkg add kirasok/epub-preview
ya pkg add Lil-Dank/lazygit
ya pkg add boydaihungst/mediainfo
ya pkg add yazi-rs/plugins:piper
ya pkg add yazi-rs/plugins:toggle-pane
ya pkg add yazi-rs/plugins:git
ya pkg add AminurAlam/yazi-plugins:preview-typst
ya pkg add AminurAlam/yazi-plugins:font-sample
ya pkg add AminurAlam/yazi-plugins:preview-cbz
ya pkg add Ape/open-with-cmd
# Only Arch
ya pkg add NoponyAsked/sxiv
ya pkg add lpnh/fr
ya pkg add uhs-robert/recycle-bin
ya pkg add yazi-rs/plugins:chmod
ya pkg add yazi-rs/plugins:mount
```

- #dotfiles [yazi/](https://github.com/scillidan/dotfiles/tree/main/.config/yazi)